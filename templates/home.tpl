...
{include file="Global/$phpNamespace.js"}
...
Ext.onReady(function() {
	 ...
	 *
	 */
	var {$phpNamespace}Class = function (config) {
		config = config || {};
		{$phpNamespace}Class.superclass.constructor.call(this, config);
	};
	...
	EB.constructExtendRegister(
		{$phpNamespace}.page,
		'{$a}',
		'{$cmpNamespace}-page-{$a|lower}',
		{
			components: [{
				xtype: '{$cmpNamespace}-panel-{$a|lower}',
	 ...
	 */
	MODx.load({ xtype: '{$cmpNamespace}-page-{$a|lower}'});
});
</script>
