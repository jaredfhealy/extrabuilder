...
var tpBuild = {
	text: _("{$namespace|lower}.{$gridClass|lower}.build_transport"),
...
// Add resolver option
var tpAddResolver = {
	text: _("{$namespace|lower}.{$gridClass|lower}.add_resolver_menu"),
	handler: function(_this) {
		// Get the record
		var grid = Ext.getCmp("{$namespace|lower}-grid-{$gridClass|lower}");
		if (grid) {
...
var tpAddTablesResolver = {
	text: _("{$namespace|lower}.{$gridClass|lower}.add_tables_resolver_menu"),
	handler: function(_this) {
		// Get the record
		var grid = Ext.getCmp("{$namespace|lower}-grid-{$gridClass|lower}");
		if (grid) {
...
var tpAddRemoveTablesResolver = {
	text: _("{$namespace|lower}.{$gridClass|lower}.add_remove_tables_resolver_menu"),
	handler: function(_this) {
		// Get the record
		var grid = Ext.getCmp("{$namespace|lower}-grid-{$gridClass|lower}");
		if (grid) {
...
var tpBackupElements = {
	text: _("{$namespace|lower}.{$gridClass|lower}.backup_elements"),
	handler: function(_this) {
		// Get the record
		var grid = Ext.getCmp("{$namespace|lower}-grid-{$gridClass|lower}");
		if (grid) {
