Ext.apply(EB.model['{$gridClass}'].grid.config, {
	...
	columns: [
		...
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.package_short"),
			dataIndex: "package",
			sortable: false,
			editor: {
		...
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.category_short"),
			dataIndex: "category",
			sortable: false,
			editor: {
		...
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.major_short"),
			dataIndex: "major",
			sortable: false,
		},
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.minor_short"),
			dataIndex: "minor",
			sortable: false,
		},
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.patch_short"),
			dataIndex: "patch",
			sortable: false,
		},
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.release_short"),
			dataIndex: "release",
			sortable: false,
			editor: {
		...
		{
			header: _("{$namespace|lower}.{$gridClass|lower}.release_index_short"),
...
Ext.apply(EB.model['{$gridClass}'].grid.overrides, {
	...
	buildTransport: function (transportId) {
		...
		Ext.Ajax.request({
			...
			params: { 
				action: '{$namespace}\\Processors\\ebTransport\\Build',
				id: transportId
			},
	...
	addResolver: function (transportId) {
		...
		Ext.MessageBox.prompt(
			...
			_("{$namespace|lower}.{$gridClass|lower}.add_resolver_prompt"),
			function(btn, text) {
				...
				if (btn === 'ok') {
					...
					Ext.Ajax.request({
						url: {$namespace}.config.connectorUrl,
						params: { 
							action: '{$namespace}\\Processors\\ebTransport\\Actions',
							id: transportId,
